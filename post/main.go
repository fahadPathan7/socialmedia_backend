...
import (
	...
	"github.com/fahadPathan7/socialmedia_backend/post/handler"
	"github.com/fahadPathan7/socialmedia_backend/post/repository"
	"github.com/fahadPathan7/socialmedia_backend/post/setting"
...
func main() {
	...
	// Register GRPC
	pb.RegisterPostServiceServer(ms.srv, svcImpl)
	...
	err = pb.RegisterPostServiceHandlerFromEndpoint(context.Background(), gmux, ms.addr, []grpc.DialOption{grpc.WithInsecure()})
	if err != nil {
		log.Fatalf("could not register grpc gateway: %v", err)
	}

	// Enable swagger
	err = enableSwagger(ms.mux, "/docs/", "./proto/post/post.swagger.json")
	if err != nil {
