...
import (
	...
	"github.com/fahadPathan7/socialmedia_backend/react/handler"
	"github.com/fahadPathan7/socialmedia_backend/react/repository"
	"github.com/fahadPathan7/socialmedia_backend/react/setting"
...
func main() {
	...
	// Register GRPC
	pb.RegisterReactServiceServer(ms.srv, svcImpl)
	...
	err = pb.RegisterReactServiceHandlerFromEndpoint(context.Background(), gmux, ms.addr, []grpc.DialOption{grpc.WithInsecure()})
	if err != nil {
		log.Fatalf("could not register grpc gateway: %v", err)
	}

	// Enable swagger
	err = enableSwagger(ms.mux, "/docs/", "../proto/react/react.swagger.json")
	if err != nil {
