...
import (
	...
	"github.com/fahadPathan7/socialmedia_backend/comment/handler"
	"github.com/fahadPathan7/socialmedia_backend/comment/repository"
	"github.com/fahadPathan7/socialmedia_backend/comment/setting"
...
func main() {
	...
	// Register GRPC
	pb.RegisterCommentServiceServer(ms.srv, svcImpl)
	...
	err = pb.RegisterCommentServiceHandlerFromEndpoint(context.Background(), gmux, ms.addr, []grpc.DialOption{grpc.WithInsecure()})
	if err != nil {
		log.Fatalf("could not register grpc gateway: %v", err)
	}

	// Enable swagger
	err = enableSwagger(ms.mux, "/docs/", "../proto/comment/comment.swagger.json")
	if err != nil {
